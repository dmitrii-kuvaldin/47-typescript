...
export default function Lesson08() {
  ...
  useEffect(() => {
    console.log('этот вывод в консоль сработает в начале и при изменении toggle, когда прячем параграф');
  }, [toggle]);
  ...
  return (
    <div className='lesson-container'>
      ...
      <p>Обновления состояния в react компоненте включает rerender компонента (update). В результате действия в теле функции сработают еще раз (смотрите консоль): </p>

      <div>{count}</div>
      <MyButton text='increase' onClick={handleIncrease} />
      <p>Когда мы хотим задать для действий внутри react компонентов особые условия для выполнения мы используем хук useEffect()</p>
      <MyButton isPrimary={true} text='hide paragraph bellow' onClick={handleToggle} />
      {/* если toggle true то блок кода снизу будет показан */}
      {toggle && (
        <>
          <p>С помощью него мы можем изолировать действия от многократного повторения, например, вызывать их только в самом начале жизненного цикла компонента - mounting</p>
        </>
      )}
